---
title: "Development"
description: "Build from source, run tests, and contribute to ccx-mem"
---
...
2. Bundles the unified hook/worker entry to `plugin/scripts/worker-service.cjs`
3. Bundles the legacy MCP compatibility server to `plugin/scripts/mcp-server.cjs`
4. Bundles web viewer UI to `plugin/ui/viewer.html`
5. Syncs supporting runtime scripts such as `bun-runner.js` and `smart-install.js`

**Build Output**:
- Unified hook + worker entry: `worker-service.cjs` (CJS format)
- Smart installer: `smart-install.js` (ESM format)
- Bun bootstrap wrapper: `bun-runner.js` (ESM format)
- MCP compatibility server: `mcp-server.cjs` (CJS format)
...
```
src/
├── cli/             # Hook adapters, unified entry, and lifecycle handlers
├── hooks/           # Lightweight shared hook response helpers
├── services/        # Worker service, HTTP routes, search, and database
├── servers/         # Legacy MCP compatibility server
├── sdk/             # Structured prompt / response helpers
...
# Test the unified hook entry manually
...
### Adding or Changing Hook Behavior

Current hook behavior should be implemented through the unified entry and lifecycle handlers.

1. Update the relevant handler in `src/cli/handlers/`
2. If needed, update dispatch logic in `src/cli/hook-command.ts`
3. If the runtime wiring changes, update `plugin/hooks/hooks.json`
4. Rebuild and test through `plugin/scripts/worker-service.cjs hook claude-code <event>`

**Note**: `src/hooks/` is no longer the main hook implementation area. It mainly contains lightweight shared hook response helpers.
...
### Extending Search or Retrieval Behavior

For current development, extend the worker-backed retrieval stack directly.

Typical touch points are:
1. search logic in `src/services/worker/SearchManager.ts`
2. HTTP wiring in `src/services/worker/http/routes/SearchRoutes.ts`
3. persistence/query support in `src/services/sqlite/SessionSearch.ts` or related SQLite modules
4. response formatting in the worker or context-generation layer where needed
...
ccx-ecc-mem relies on **real-world usage and manual testing** rather than traditional unit tests. The project philosophy prioritizes:
...
**Hook Testing**:
```bash
# Test the unified hook entry for context injection
printf '{}' | node plugin/scripts/worker-service.cjs hook claude-code context

# Test the unified hook entry for prompt intake
...
Run the unified hook entry manually with test input:

```bash
# Test context injection path
printf '{}' | node plugin/scripts/worker-service.cjs hook claude-code context

# Test prompt-submission path
printf '{}' | node plugin/scripts/worker-service.cjs hook claude-code user-prompt-submit
```
...
- [Configuration](configuration) - Customize ccx-mem
