---
title: "Configuration"
description: "Environment variables and settings for ccx-ecc-mem"
---
...
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_MODEL`            | `claude-sonnet-4-6`             | Claude model alias used by compatible custom APIs when processing observations |
| `CLAUDE_MEM_MODE`             | `code--zh`                      | Active mode profile and default language |
...
| `CLAUDE_MEM_CHROMA_ENABLED`   | `false`                         | Enable optional Chroma semantic retrieval; SQLite search remains active when false |
| `CLAUDE_MEM_TRANSCRIPTS_ENABLED` | `false`                      | Enable Codex CLI transcript ingestion; the Codex installer sets this to `true` |

### Custom API Settings

| Setting                               | Default                               | Description |
|---------------------------------------|---------------------------------------|-------------|
| `CLAUDE_MEM_CUSTOM_BASE_URL`          | —                                     | OpenAI-compatible chat completions endpoint used by the worker |
| `CLAUDE_MEM_CUSTOM_API_KEY`           | —                                     | API key used for custom API requests |
| `CLAUDE_MEM_CUSTOM_MODEL`             | —                                     | Model identifier sent to the custom API |
| `CLAUDE_MEM_CUSTOM_APP_NAME`          | `ccx-mem`                             | Optional application name sent as the request title |
| `CLAUDE_MEM_CUSTOM_MAX_CONTEXT_MESSAGES` | `20`                               | Maximum recent messages retained before truncation |
| `CLAUDE_MEM_CUSTOM_MAX_TOKENS`        | `100000`                              | Estimated token budget used when truncating conversation history |
| `CLAUDE_MEM_CUSTOM_TIMEOUT_MS`        | `120000`                              | Request timeout in milliseconds |
| `CLAUDE_MEM_CUSTOM_TEMPERATURE`       | `0.3`                                 | Sampling temperature sent to the custom API |

ccx-ecc-mem currently uses a single worker-side custom API path built around an OpenAI-compatible chat completions request/response shape. Configure these fields in `~/.claude-mem/settings.json` to point observation extraction at your own compatible endpoint.
...
| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_MODE` | `code--zh` | Defines behavior and language. See [Modes & Languages](modes). |

### Examples

**Chinese Code Mode (default):**
```json
{
...
├── .mcp.json               # Optional retrieval compatibility surface
├── hooks/
│   └── hooks.json          # Hook configuration
├── scripts/                # Built executables
│   ├── smart-install.js    # Smart installer script
│   ├── bun-runner.js       # Bun bootstrap wrapper
│   ├── worker-service.cjs  # Worker service + hook entry (CJS)
│   └── mcp-server.cjs      # Legacy MCP retrieval bridge (CJS)
└── ui/
...
Hooks are configured in `plugin/hooks/hooks.json` and currently route through the unified worker hook entry:
...
ccx-ecc-mem provides worker-backed retrieval for querying your project history.

The current runtime reads and serves stored context through the worker HTTP API and viewer surfaces. Where enabled, legacy MCP tooling remains available as a compatibility surface.

Retrieval operations are available via:
- **Worker HTTP API**: Search and observation endpoints on the worker service port
- **Viewer UI**: Interactive inspection of prompts, observations, and summaries
- **Compatibility Surfaces**: MCP-based retrieval where still enabled

## Version Channel

ccx-ecc-mem supports switching between stable and beta versions via the web viewer UI.
...
ccx-ecc-mem can automatically generate `CLAUDE.md` files in your project folders with activity timelines. This feature is disabled by default.
...
ccx-ecc-mem injects past observations into each new session, giving Claude awareness of recent work. You can configure exactly what gets injected using the **Context Settings Modal**.
...
| Setting | Default | Description |
|---------|---------|-------------|
| **Model** | sonnet | AI model alias used for observation extraction |
| **Worker Port** | 37777 | Port for background worker service |
| **Compatibility MCP server** | true | Enable the legacy MCP compatibility surface when needed |
