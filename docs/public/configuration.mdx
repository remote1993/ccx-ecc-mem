...
|-------------------------------|---------------------------------|---------------------------------------|
| `CLAUDE_MEM_MODEL`            | `sonnet`                        | Claude model alias used by compatible custom APIs when processing observations |
...
| Setting                               | Default                               | Description |
|---------------------------------------|---------------------------------------|-------------|
| `CLAUDE_MEM_CUSTOM_BASE_URL`          | `https://openrouter.ai/api/v1/chat/completions` | OpenAI-compatible chat completions endpoint used by the worker |
| `CLAUDE_MEM_CUSTOM_API_KEY`           | —                                     | API key used for custom API requests |
| `CLAUDE_MEM_CUSTOM_MODEL`             | `xiaomi/mimo-v2-flash:free`           | Model identifier sent to the custom API |
| `CLAUDE_MEM_CUSTOM_APP_NAME`          | `claude-mem`                          | Optional application name sent as the request title |
...
Claude-mem currently uses a single worker-side custom API path built around an OpenAI-compatible chat completions request/response shape. Configure these fields in `~/.claude-mem/settings.json` to point observation extraction at your own compatible endpoint.
...
Claude-Mem provides worker-backed retrieval for querying your project history.
