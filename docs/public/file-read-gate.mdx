...
description: "How ccx-ecc-mem intercepts file reads to save tokens using observation history"
---
...
2. Find the ccx-ecc-mem hooks section under `hooks.PreToolUse` and remove the entry with the `Read` matcher.
...
The File Read Gate is one piece of ccx-ecc-mem's layered context strategy:
