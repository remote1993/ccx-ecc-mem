...
Knowledge agents let you compile a slice of your ccx-ecc-mem observation history into a **queryable "brain"** that answers questions conversationally. Instead of getting raw search results back, you get synthesized, grounded answers drawn from your actual project history -- decisions, discoveries, bugfixes, and features.
...
**Key insight:** the worker HTTP API is the current entry surface for corpus operations. If compatibility MCP tools still exist in some environments, treat them as optional wrappers rather than the main architecture story.
