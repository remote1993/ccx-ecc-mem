...
This document describes the PM2 to Bun migration that occurred in v7.1.0 (December 2025). If you're installing ccx-ecc-mem for the first time, this migration has already been completed and you can use the current Bun-based system documented in the main guides.
...
ccx-ecc-mem version 7.1.0 introduces two major architectural migrations:
...
# 2. Start old PM2 worker from that checked-out version
pm2 start <old-worker-entry> --name claude-mem-worker
